...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  // Defense in depth: every external call below is meant to be individually
  // handled, but a genuinely unexpected throw (or a future unguarded call)
  // must still return clean JSON rather than reaching Vercel's runtime as a
  ...
  try {
    ...
    console.error("[reservations] unhandled error:", err);
    if (!res.headersSent) {
      return res.status(500).json({ error: "Something went wrong. Please try again." });
    }
  }
